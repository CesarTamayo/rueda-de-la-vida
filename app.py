...
def enviar_correo(email_destino, nombre_usuario, figura):
    ...
    nombre_archivo = f"Rueda_Vida_{nombre_usuario.replace(' ', '_')}.pdf"
    ...
    # st.secrets lee las credenciales seguras de tu configuración en Streamlit Cloud
    ...
    # 4. Conectarse al servidor de Gmail y enviar (Requiere contraseña de aplicación)
...
fig.update_layout(
    ...
    showlegend=False,
    title=dict(text=f"Rueda de la Vida de {nombre_display}", font=dict(size=20))
)
    ...
    if not correo:
        ...
        with st.spinner("Generando PDF y enviando correo..."):
            try:
                ...
                # st.write(e) # Descomenta esta línea si necesitas ver el error técnico exacto en pantalla
